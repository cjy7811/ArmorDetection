...
private:
    // 相机内参
    Mat cameraMatrix = (Mat_<double>(3, 3) << 9.28130989e+02, 0, 3.77572945e+02, 0, 9.30138391e+02, 2.83892859e+02, 0, 0, 1.0000);
    // 畸变系数
    Mat distCoeffs = (Mat_<double>(1, 5) << -2.54433647e-01, 5.69431382e-01, 3.65405229e-03, -1.09433818e-03, -1.33846840e+00);
...
public:
    ...
    Mat preprocess(Mat &frame, bool isRed)
    {
        ...
        GaussianBlur(hsv, hsv, Size(5, 5), 0); // 高斯模糊
    ...
    vector<LightDescriptor> detectLights(Mat &binary, Mat &frame)
    {
        ...
        for (size_t i = 0; i < contours.size(); i++)
        {
            ...
            if (lightRect.size.width > lightRect.size.height)
            {
                ...
                if (lightRect.angle > 180.0f)
                    lightRect.angle -= 180.0f;
            }
        ...
        for (int a = 0; a < n; ++a)
        {
            ...
            for (int b = a + 1; b < n; ++b)
            {
                ...
                float angA = rects[a].angle;
                if (angA > 90.0f)
                    angA -= 180.0f;
                float angB = rects[b].angle;
                if (angB > 90.0f)
                    angB -= 180.0f;
                ...
                if (dist < 0.6f * meanH && angDiff < 25.0f)
                {
                    ...
                    if (mergedPts.size() >= 5)
                    {
                        ...
                        if (newRect.size.width > newRect.size.height)
                        {
                            ...
                            newRect.angle += 90.0f;
                            if (newRect.angle > 180.0f)
                                newRect.angle -= 180.0f;
                        }
                        rects[a] = newRect;
        ...
        for (size_t i = 0; i < lights.size(); ++i)
        {
            for (size_t j = 0; j < lights.size(); ++j)
            {
                ...
                {
                    // j 更暗且更小 -> 视为反射
                    if (lights[j].meanV < lights[i].meanV && lights[j].height < 0.95f * lights[i].height)
                    {
                        isRef[j] = true;
                    }
                }
    ...
    vector<vector<Point2f>> matchArmor(vector<LightDescriptor> &lights, Mat &frame)
    {
        ...
        for (size_t i = 0; i < lights.size(); i++)
        {
            for (size_t j = i + 1; j < lights.size(); j++)
            {
                ...
                if (angleDiff > 15.0f)
                    reason = "angle";
                else if (heightDiffRel > 0.6f)
                    reason = "hDiff";
                else if (xDiff < 1.2f * max(leftLight.width, rightLight.width))
                    reason = "xTooClose";
                else if (yDiff > meanHeight * 0.6f)
                    reason = "yOffset";
                else if (ratio < 0.8f || ratio > 6.0f)
                    reason = "ratio";
    ...
    vector<Point2f> getArmorCorners(LightDescriptor &left, LightDescriptor &right)
    {
        ...
        if (leftAngleDeg > 90.0)
            leftAngleDeg -= 180.0;
        if (rightAngleDeg > 90.0)
            rightAngleDeg -= 180.0;
        ...
        if (armorPoints.size() == 4)
        {
            ...
            if (!rvec.empty() && !tvec.empty())
            {
                ...
                if (imgPts.size() >= 4)
                {
                    ...
                    drawAxis(imgPts[1], Scalar(0, 0, 255));
                    drawAxis(imgPts[2], Scalar(0, 255, 0));
                    drawAxis(imgPts[3], Scalar(255, 0, 0));
